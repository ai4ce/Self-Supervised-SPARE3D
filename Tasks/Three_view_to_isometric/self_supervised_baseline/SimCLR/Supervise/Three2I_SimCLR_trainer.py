...
parser.add_argument('--Training_dataroot', default="/home/wenyuhan/project/Train_dataset/Task_1_train_modify/",
                    required=False, help='path to training dataset')
parser.add_argument('--Validating_dataroot', default="/home/wenyuhan/project/Train_dataset/Task_1_eval_modify/",
                    required=False, help='path to validating dataset')
parser.add_argument('--model_root', default="/home/wenyuhan/project/Train_dataset/Task_1_eval_modify/", required=False,
...
task_1_model = Three2I_SimCLR(opt.model_type, opt.pretrained).to(opt.device)
save_model = torch.load(os.path.join(opt.model_root, 'vgg16_Lr_1e-05.pth'))
model_dict = task_1_model.model_f.state_dict()
model_dict_f =task_1_model.model_f.state_dict()
model_dict_r =task_1_model.model_r.state_dict()
model_dict_t =task_1_model.model_t.state_dict()
model_dict_i =task_1_model.model_i.state_dict()
state_dict_f = {k:v for k,v in save_model.items() if k in model_dict_f.keys()}
state_dict_r = {k:v for k,v in save_model.items() if k in model_dict_r.keys()}
state_dict_t = {k:v for k,v in save_model.items() if k in model_dict_t.keys()}
state_dict_i = {k:v for k,v in save_model.items() if k in model_dict_i.keys()}
model_dict_f.update(state_dict_f)
task_1_model.model_f.load_state_dict(model_dict_f)
model_dict_r.update(state_dict_r)
task_1_model.model_r.load_state_dict(model_dict_r)
model_dict_t.update(state_dict_t)
task_1_model.model_t.load_state_dict(model_dict_t)
model_dict_i.update(state_dict_i)
task_1_model.model_i.load_state_dict(model_dict_i)
...
def train_model():
    ...
    data_train = torch.utils.data.DataLoader(train_data, batch_size=opt.batchSize, pin_memory=True,
...
def Eval():
    ...
    data_eval = torch.utils.data.DataLoader(eval_data, batch_size=opt.batchSize, pin_memory=True,
                                            num_workers=opt.num_workers, shuffle=True)
    with torch.no_grad():
        task_1_model.eval()
